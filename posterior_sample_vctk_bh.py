...
def posterior_sample(cfg):
    ...
    # inference
    output_dir = os.path.join("results_vctk_new_model_2_spk_all", task_config.operator.name) # results_vctk_820k_finetune_grad_spk_condition
    ...
    for batch_start in range(0, len(audio_sample_group), batch_size):
        if batch_start + batch_size > len(audio_sample_group):
            batch_size = len(audio_sample_group) - batch_start
...
def degradation(x: torch.Tensor) -> torch.Tensor:
    n_spk = 2  # number of speakers, modify as needed
    B = x.shape[0] // n_spk
