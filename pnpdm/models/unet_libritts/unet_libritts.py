...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            x = F.interpolate(
                x, (x.shape[2], x.shape[3] * 2, x.shape[4] * 2), mode="linear"
            )
        else:
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                enc_layers: list[Union[ResBlock, AttentionBlock]] = [
                    ResBlock(
                        ch,
                        time_embed_dim,
                        dropout,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                dec_layers: list[Union[ResBlock, AttentionBlock, Upsample]] = [
                    ResBlock(
                        ch + ich,
                        time_embed_dim,
                        dropout,
                ...
                if level and i == num_res_blocks:
                    ...
                    ds //= 2
                self.output_blocks.append(TimestepEmbedSequential(*dec_layers))
                self._feature_size += ch
    ...
    def forward(self, x, timesteps, y=None, mask_batch=None,
                ...
                mask_ref: Optional[th.Tensor]=None,
                **kwargs,):
        """
