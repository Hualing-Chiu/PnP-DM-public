...
class SourceSeparation(NonLinearOperator):
    def __init__(self, n_spk, device):
        self.n_spk = n_spk
        self.device = device
    ...
    def proximal_generator_bh(self, x, y, diffusion, i, sigma, rho, gamma=1e-4):
        x = x.to(self.device)
        y = y.to(self.device)
        # z = x.clone().detach()
        # n_spk = z.shape[0]
        ...
        for _ in range(3):
            ...
            ortho_loss = self.compute_ortho_loss_bh(embedding.squeeze(1))
    ...
    def compute_ortho_loss_bh(self, z_):
        """
    ...
    def compute_ortho_loss(self, z_):
        ...
        根據論文公式 ||V^T V - I||_F^2 實作 z_ 為 [B, D] 的嵌入矩陣。
        """
        z_norm = F.normalize(z_, p=2, dim=-1)  # 每個 embedding 單位化 [n_spk, D]
        dot_matrix = torch.matmul(z_norm, z_norm.T)  # V^T V, shape = [n, n]
    ...
    def compute_adaptive_rho(self, rho, i, loss_value):
        ...
        return rho * loss_factor
    
    # def save_heatmap(self, matrix, path, title="Heatmap"):
    #     plt.figure(figsize=(6, 5))
    #     sns.heatmap(matrix, cmap='Reds', vmin=0, vmax=1, annot=True, fmt=".2f",)
    #     plt.title(title)
    #     plt.tight_layout()
    #     plt.savefig(path)
    #     plt.close()
